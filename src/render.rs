...
pub fn render_scanlines_bin_solid<C, T>(
...
pub fn render_scanlines_aa_solid<C, T>(
    ras: &mut RasterizerScanline,
    ren: &mut RenderingBase<T>,
    color: C,
) where
        ...
        if let Some(clip_box) = self.clip_box {
            ...
            if (flags & 4) == 0 {
                if flags != 0 {
                    ...
                    if flags & 1 != 0 {
                        self.start += (len_i64_xy(lp.x1, lp.y1, x1, y1) as f64
                            / self.scale_x as f64)
                            .round() as i64;
                        sx = x1 + (y2 - y1);
                        sy = y1 - (x2 - x1);
        ...
        T: Source + Pixel,
    {
        ...
        for y in 0..h as usize {
            ...
            for x in 0..self.dilation as usize {
                ...
                self.pix
                    .set((dx2 - x - 1, y), self.pix.get((sx2 - x - 1, y)));
            }
        }
    }
