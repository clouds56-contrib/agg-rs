...
fn render_scanline_bin_solid<T: Pixel, C: Color>(sl: &ScanlineU8, ren: &mut RenderingBase<T>, color: C) {
  for span in &sl.spans {
    ren.blend_hline(span.x, sl.y, span.x - 1 + span.len.abs(), color, Cover::Full);
  }
}
  ...
  G::Color: Color,
{
  ...
  for span in &sl.spans {
    ...
    let covers = span.covers.iter().map(|i| i.into_()).collect::<Vec<_>>();
    if len < 0 {
      len = -len;
    }
    //dbg!(x);
...
pub fn render_all_paths<REN, VS, C>(ras: &mut RasterizerScanline, ren: &mut REN, paths: &[VS], colors: &[C])
where
  C: Color,
  REN: Render,
  VS: VertexSource,
...
impl<T> DrawOutline for RendererOutlineImg<'_, T>
where
  T: Pixel,
{
  ...
  fn semidot<F>(&mut self, _cmp: F, _xc1: i64, _yc1: i64, _xc2: i64, _yc2: i64)
  where
    F: Fn(i64) -> bool,
  {
  }
...
impl<'a, T> RendererOutlineImg<'a, T>
where
  T: Pixel,
{
  ...
  fn blend_color_hspan(&mut self, x: i64, y: i64, len: i64, colors: &[Rgba8]) {
    self.ren.blend_color_hspan(x, y, len, colors, &[], Cover::Full);
  }
  fn blend_color_vspan(&mut self, x: i64, y: i64, len: i64, colors: &[Rgba8]) {
    self.ren.blend_color_vspan(x, y, len, colors, &[], Cover::Full);
  }
...
impl LineImagePatternPow2 {
  pub fn new(filter: PatternFilterBilinear) -> Self {
    ...
    Self {
      base,
      mask: POLY_SUBPIXEL_MASK as u64,
    }
  }
  pub fn create<T>(&mut self, src: &T)
  where
...
impl LineInterpolatorImage {
    ...
    T: Pixel,
  {
    ...
    ren.blend_color_hspan(self.x - dx as i64 + 1, self.y, (p1 - p0) as i64, &self.colors[p0..p1]);
    self.step += 1;
    ...
    T: Pixel,
  {
    ...
    ren.blend_color_vspan(self.x, self.y - dy as i64 + 1, (p1 - p0) as i64, &self.colors[p0..p1]);
    self.step += 1;
    npix != 0 && self.step < self.count
  }
}
