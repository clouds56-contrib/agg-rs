...
impl GsvText {
    pub fn new() -> Self {
        Self {
            ...
            space: 0.0,
            line_space: 0.0,
            font: GsvDefaultFont::get().to_vec(),
    ...
    fn xconvert(&self) -> Vec<Vertex<f64>> {
        ...
        loop {
            match status {
                ...
                TextStatus::NextChar => match chars.next() {
                    None => break,
                    Some((_, chr)) => {
                        if chr == '\n' {
                            x = self.start_x;
...
fn string_width(txt: &str, font: &ft::Face) -> f64 {
    ...
    for c in txt.chars() {
        if let Some(glyph_index) = font.get_char_index(c as usize) {
            font.load_glyph(glyph_index, ft::face::LoadFlag::DEFAULT)
                .unwrap();
...
impl<'a> Label<'a> {
    ...
    {
        draw_text_subpixel(
            &self.txt, self.x, self.y, self.xa, self.ya, self.color, self.font, ren,
        );
    }
}
