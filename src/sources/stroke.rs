  ...
  T: VertexSource,
{
  ...
  fn calc_arc(&self, x: f64, y: f64, dx1: f64, dy1: f64, dx2: f64, dy2: f64) -> Vec<Vertex<f64>> {
    ...
    if self.width_sign > 0.0 {
      ...
      // Create Arc
      for _ in 0..n {
        out.push(Vertex::line_to(x + a1.cos() * self.width, y + a1.sin() * self.width));
        a1 += da;
      }
    } else {
      // Negative Line Width
  ...
  fn calc_miter(
    ...
    //      \  \
    //       \2 d
    if let Some((xit, yit)) = self.calc_intersection(
      p0.x + dx1,
      p0.y - dy1, // a
      p1.x + dx1,
      p1.y - dy1, // b
      ...
      match join {
        ...
        _ => {
          ...
          if intersection_failed {
            ...
            out.push(Vertex::line_to(p1.x + dx1 + dy1 * mlimit, p1.y - dy1 + dx1 * mlimit));
            out.push(Vertex::line_to(p1.x + dx2 - dy2 * mlimit, p1.y - dy2 - dx2 * mlimit));
          } else {
            let x1 = p1.x + dx1;
            let y1 = p1.y - dy1;
            let x2 = p1.x + dx2;
            ...
            out.push(Vertex::line_to(x1 + (xi - x1) * di, y1 + (yi - y1) * di));
            out.push(Vertex::line_to(x2 + (xi - x2) * di, y2 + (yi - y2) * di));
          }
        }
      }
    }
  ...
  fn calc_join(&self, p0: &Vertex<f64>, p1: &Vertex<f64>, p2: &Vertex<f64>) -> Vec<Vertex<f64>> {
    ...
    if cp != 0.0 && cp.is_sign_positive() == self.width.is_sign_positive() {
        ...
        && self.approx_scale * (self.width_abs - dbevel) < self.width_eps
      {
        ...
        if let Some((dx, dy)) = self.calc_intersection(
          p0.x + dx1,
          p0.y - dy1,
          p1.x + dx1,
          p1.y - dy1,
      ...
      match self.line_join {
        LineJoin::Miter | LineJoin::MiterRevert | LineJoin::MiterRound => {
          out.extend(self.calc_miter(p0, p1, p2, dx1, dy1, dx2, dy2, self.line_join, self.miter_limit, dbevel))
        }
        LineJoin::Round => out.extend(self.calc_arc(p1.x, p1.y, dx1, -dy1, dx2, -dy2)),
        LineJoin::Bevel => {
          out.push(Vertex::line_to(p1.x + dx1, p1.y - dy1));
  ...
  fn stroke(&self) -> Vec<Vertex<f64>> {
    ...
    for (m1, m2) in pairs {
      ...
      for i in n1..n2 {
        // Forward Path
        outf.extend(self.calc_join(&v[prev!(i, n)], &v[curr!(i, n)], &v[next!(i, n)]));
      }
      if closed {
        // Close the polygon
        let n = outf.len();
      ...
      for i in (n1..n2).rev() {
        // Backward Path
        outb.extend(self.calc_join(&v[next!(i, n)], &v[curr!(i, n)], &v[prev!(i, n)]));
      }
      if closed {
