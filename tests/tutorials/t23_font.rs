...
fn t23_font() {
  ...
  assert_eq!(font.family_name().unwrap(), "Helvetica");
  assert_eq!(font.style_name().unwrap(), "Regular");
  font.set_pixel_sizes(0, 13).unwrap();
  assert_eq!(line_height(&font), 14.0);
  assert_eq!(font.height(), 2048);
